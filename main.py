...
DATE_TO_CHECK = TODAY.replace(month=((TODAY.month - 1) + 3) % 12 + 1)
ORIGIN = 'YIA'
...
# making function to send e-mail
...
for something in sheety_read['sheet1']:
    ...
    if int(lowest_price) > cheapest:
        send_email(EMAIL_TO, 'Low Price Alert!', f'Only {cheapest} with {its_duration} flight duration to fly to {city_name} from Yogyakarta! in {DATE_TO_CHECK}')
